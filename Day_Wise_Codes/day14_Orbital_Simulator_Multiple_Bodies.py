# In day 14, we'll be using our orbit simulator and add all elemnts into it 
# Now the system contains an integrator engine, a controller able to burn under given conditions 
# It also contains an earth only and an earth moon model
# 
...
# region Choosing the integrator and system model
...
if use_moon:
    r_earth=np.sqrt(x**2+y**2)
    r_moon=np.sqrt((x-x_moon)**2+(y-y_moon)**2)
    E=0.5*(np.array(vx)**2+np.array(vy)**2) - mu_earth/r_earth - mu_moon/r_moon
    L=(np.array(x)*np.array(vy)-np.array(y)*np.array(vx))

else:
    r_earth=np.sqrt(np.array(x)**2+np.array(y)**2)
    E=0.5*(np.array(vx)**2+np.array(vy)**2) -mu_earth/r_earth
    L=(np.array(x)*np.array(vy)-np.array(y)*np.array(vx))
